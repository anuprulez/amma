...
These different analyses were done following the same procedure. The differentially expressed genes (DEGs) showing adjusted p-values (Wald test) less than 0.05 and absolute fold change greater than 1.5 were identified. The heatmaps for Z-scores of DEGs are plotted using `pheatmap` (1.0.8) with hierarchical clustering of the rows (complete method). The enrichment analyses (Gene Ontology and KEGG) for the DEGs were performed using `goseq` (1.26.0, {% cite Young2010 %}), with the genome wide annotation for Mouse `org.Mm.eg.db` (3.4.0) and the Wallenius approximation. The over and under enriched categories were extracted using a 0.05 FDR cutoff ({% Benjamini1995 %}). Networks of GO terms were generated using `RamiGO` (1.20.0, {% cite Markus2013 %}) and the KEGG pathways using `pathview` (1.14.0, {% cite Luo2013 %}).
